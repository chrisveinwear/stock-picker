...
/**
 * The live buy/hold/sell zone for every watch-listed ticker with a research
 * report — the single source the Action Alerts page, the alerts API and the
 * dashboard all count from. Thresholds come from the latest report row
 * (resolved + clamped at save time, see lib/report-thresholds.ts); prices are
 * live quotes for equities and report-date native-unit spot for commodities.
 */
...
export async function getResearchAlerts(): Promise<ResearchAlert[]> {
  const db = getDb();
  const rows = db
    .select()
